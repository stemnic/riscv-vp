...
public:
	...
	addr_t mem_size = 1024u * 1024u * 32;  // 1024 MB ram
	...
	addr_t uart0_end_addr = 0x10000fff;
...
int sc_main(int argc, char **argv) {
	...
	SimpleBus<NUM_CORES + 1, 5> bus("SimpleBus");
	...
	for (size_t i = 0; i < NUM_CORES; i++) {
		cores[i]->init(opt.use_data_dmi, opt.use_instr_dmi, &clint, entry_point, rv32_align_address(opt.mem_end_addr));
	...
	bus.ports[3] = new PortMapping(opt.uart0_start_addr, opt.uart0_end_addr);
	bus.ports[4] = new PortMapping(opt.plic_start_addr, opt.plic_end_addr);
	...
	bus.isocks[3].bind(uart0.tsock);
	bus.isocks[4].bind(plic.tsock);

...
class RedVOptions : public Options {
public:
	typedef unsigned int addr_t;

	addr_t maskROM_start_addr = 0x00010000;
	addr_t maskROM_end_addr = 0x00011FFF;
	addr_t clint_start_addr = 0x02000000;
	addr_t clint_end_addr = 0x0200FFFF;
	addr_t sys_start_addr = 0x02010000; // not from datasheet?
	addr_t sys_end_addr = 0x020103ff;
	addr_t plic_start_addr = 0x0C000000;
	addr_t plic_end_addr = 0x0FFFFFFF;
	addr_t aon_start_addr = 0x10000000;
	addr_t aon_end_addr = 0x10000FFF;
	addr_t prci_start_addr = 0x10008000;
	addr_t prci_end_addr = 0x10008FFF;
	...
	addr_t i2c_start_addr = 0x10016000;
	addr_t i2c_end_addr = 0x10016FFF;	
	...
	addr_t itim_size = 1024 * 8;  // 16 KB dram
	addr_t itim_start_addr = 0x08000000;
	addr_t itim_end_addr = itim_start_addr + itim_size - 1;
...
int sc_main(int argc, char **argv) {
	...
	MemoryDMI itim_dmi = MemoryDMI::create_start_size_mapping(itim.data, opt.itim_start_addr, itim.size);
	...
	bus.ports[14] = new PortMapping(opt.i2c_start_addr, opt.i2c_end_addr);
	bus.ports[15] = new PortMapping(opt.itim_start_addr, opt.itim_end_addr);
...
//ToDo sys and core with more than one ram for ITIM
	...
	bus.isocks[14].bind(i2c.tsock);
	bus.isocks[15].bind(itim.tsock);
